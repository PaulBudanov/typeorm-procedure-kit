...
| T21    | `a90ca68`            | готово; P3 · 10c уточнена: предупреждение достижимо после неудачного `initDatabase()` с неполным откатом |
...
| **T30**  | Остатки в фасаде после T21: на пути «init упал, откат неполный» `cleanupResources(false)` не вызывает `caseSettings.strategy.destroy()`, и кэш имён не чистится; `callSqlTransaction()` и геттер `databaseAdapter` до init бросают `Database adapter is not initialized`, а не сообщение кита; булев параметр `cleanupResources(isDestroyCaseStrategy: boolean)`; JSDoc `initMainClasses` перечисляет `DatabaseInitializerBase`, который метод не создаёт | `core/index.ts`, `test/core/typeorm-procedure-kit.test.ts`                                                                                                                                                       | в любой момент |
...
**T21** — в раздел «Shutdown», после предложения, оканчивающегося «…throws `AggregateError` if part of cleanup fails.»:

- `README.md`: «`destroy()` is safe to call more than once: every later call, during shutdown or after it, returns the promise of the first call, so it resolves or rejects with the same result and does not run cleanup again.»
- `docs/README.ru.md`: «`destroy()` можно вызывать повторно: каждый следующий вызов, во время shutdown или после него, возвращает promise первого вызова, поэтому завершается с тем же результатом и не запускает cleanup ещё раз.»
- `docs/README.de.md`: «`destroy()` darf mehrfach aufgerufen werden: jeder weitere Aufruf, waehrend des shutdown oder danach, gibt das promise des ersten Aufrufs zurueck, endet also mit demselben Ergebnis und startet den cleanup nicht erneut.»
- `docs/README.zh.md`: «`destroy()` 可以多次调用：之后的每次调用（无论在关闭期间还是之后）都返回第一次调用的 promise，因此结果相同，也不会再次执行 cleanup。»
- В гайд миграции: «`ServerError.unsafeGetContextAs()` was removed; read the public `error.errorContext` field instead.» README метод не упоминает.
- `CLAUDE.md` (~:43): убрать `SerializerBase` из списка баз и заодно поправить «each lazily constructed» (P3 · 31) — `initMainClasses()` создаёт их разом. Внимание: `CLAUDE.md` в `.gitignore`, правка останется локальной; решение, коммитить ли его, — за владельцем.
