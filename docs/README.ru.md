...
- `callTimeout`: slow-query threshold для underlying DataSource.
...
- `cqnPort`: callback port для server-initiated Oracle CQN.
...
- `optionsCommands`: SQL commands, выполняемые в той же transaction перед
  основным query.
