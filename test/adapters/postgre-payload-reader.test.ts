...
describe('PostgreSQL procedure payload reader', (): void => {
  ...
  describe('keys inherited from a prototype other than Object.prototype', (): void => {
    ...
    it('rejects a structured argument alias inherited from a prototype object', (): void => {
      expectInherited(
        'structured',
        Object.create({ profile: OTHER_PROFILE }) as object,
        'profile',
        'p_profile'
      );
    });
    ...
    // PostgreSQL-only: Oracle binds a REF CURSOR without reading the payload,
    // so this call site has no Oracle counterpart.
    it('rejects a refcursor argument alias inherited from a prototype object', (): void => {
      expectInherited(
        'cursor',
        Object.create({ cursor: 'portal_a' }) as object,
        'cursor',
        'p_cursor'
      );
    });
