...
> extends QueryBuilder<Entity> {
  public readonly '@instanceof' = Symbol.for('InsertQueryBuilder');
        ...
        if (this.expressionMap.onIgnore) {
          ...
          if (Array.isArray(overwrite)) {
            updatePart.push(
              ...overwrite.map(
                (column) =>
                  `${this.escape(column)} = EXCLUDED.${this.escape(column)}`
              )
            );
          } else if (columns) {
            updatePart.push(
              ...columns.map((column) => `${this.escape(column)} = :${column}`)
            );
          }
          ...
          if (updatePart.length > 0) {
            ...
            updatePart.push(
              ...this.expressionMap
                ...
                .map(
                  (column) => `${this.escape(column.databaseName)} = DEFAULT`
                )
            );
          ...
          if (Array.isArray(overwrite) && skipUpdateIfNoValuesChanged) {
            ...
            const wheres = overwrite.map<WhereClause>((column) => ({
              ...
              condition: `${tableOrAliasName}.${this.escape(
                column
              )} IS DISTINCT FROM EXCLUDED.${this.escape(column)}`,
            }));
        ...
        if (this.expressionMap.onUpdate) {
          ...
          if (Array.isArray(overwrite)) {
            ...
            query += overwrite
              .map(
                (column) =>
                  `${this.escape(column)} = VALUES(${this.escape(column)})`
              )
              .join(', ');
            query += ' ';
  ...
  protected createColumnNamesExpression(): string {
    ...
    if (
      ...
      if (valueSets.length === 1)
        return Object.keys(valueSets[0] as ObjectLiteral)
  ...
  protected getValueSets(): Array<ObjectLiteral> {
    if (Array.isArray(this.expressionMap.valuesSet))
      return this.expressionMap.valuesSet as Array<ObjectLiteral>;
  ...
  protected createMergeExpression(): string {
    ...
    if (this.expressionMap.onIgnore) {
      ...
      if (primaryKey) {
        ...
        )} = ${mergeSourceAlias}.${this.escape(primaryKey.databaseName)})`;
      } else {
            ...
            return `(${unique.columns
              .map((column) => {
                return `${tableAlias}.${this.escape(
                  column.databaseName
                )} = ${mergeSourceAlias}.${this.escape(column.databaseName)}`;
              })
              .join(' AND ')})`;
          })
          .join(' OR ')})`;
