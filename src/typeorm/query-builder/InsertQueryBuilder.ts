...
import { InsertResult } from './result/InsertResult.js';
import { ReturningResultsEntityUpdator } from './ReturningResultsEntityUpdator.js';

import type { InsertOrUpdateOptions } from './InsertOrUpdateOptions.js';
...
> extends QueryBuilder<Entity> {
  public override readonly '@instanceof' = Symbol.for('InsertQueryBuilder');
   ...
   */
  public override async execute(): Promise<InsertResult> {
    // console.time(".value sets");
    const valueSets: Array<ObjectLiteral> = this.getValueSets();
    ...
    try {
      ...
      if (
        ...
        if (!(valueSets.length > 1 && this.driver.options.type === 'oracle')) {
   ...
   */
  public into<T extends ObjectLiteral>(
  ...
  public returning(returning: string | Array<string>): this {
    ...
    if (!this.driver.isReturningSqlSupported('insert')) {
   ...
   */
  public onConflict(statement: string): this {
   ...
   */
  public orIgnore(statement: string | boolean = true): this {
   ...
   *
   */
  public orUpdate(statement?: {
    columns?: Array<string>;
    overwrite?: Array<string>;
    conflict_target?: string | Array<string>;
  }): this;

  public orUpdate(
    overwrite: Array<string>,
    conflictTarget?: string | Array<string>,
  ...
  public orUpdate(
    ...
    let wheres: Array<WhereClause> | undefined;
    if (where) {
  ...
  protected createInsertExpression(): string {
    if (this.expressionMap.onUpdate || this.expressionMap.onIgnore) {
      if (
        (this.expressionMap.onUpdate?.upsertType ?? 'merge-into') ===
          'merge-into' &&
        this.driver.supportedUpsertTypes.includes('merge-into')
      )
    ...
    const tableOrAliasName =
      this.alias !== this.getMainTableName()
        ? this.escapeAlias(this.alias)
        : tableName;
    const valuesExpression = this.createValuesExpression(); // its important to get values before returning expression because oracle rely on native parameters and ordering of them is important
    const returningExpression =
      this.driver.options.type === 'oracle' && this.getValueSets().length > 1
        ? null
    ...
    if (
      ...
      query += ` AS ${this.escapeAlias(this.alias)}`;
    }
    ...
    if (valuesExpression) {
      if (
        this.driver.options.type === 'oracle' &&
    ...
    if (this.expressionMap.onUpdate?.upsertType !== 'primary-key') {
      if (this.driver.supportedUpsertTypes.includes('on-conflict-do-update')) {
        if (this.expressionMap.onIgnore) {
          ...
          if (Array.isArray(conflict)) {
            conflictTarget += ` ( ${conflict
              .map((column) => this.escapeDatabaseIdentifier(column))
              .join(', ')} )`;
            if (
              indexPredicate &&
              !DriverUtils.isPostgresFamily(this.connection.driver)
            ...
            conflictTarget += ` ON CONSTRAINT ${this.escapeAlias(conflict)}`;
          }

          const updatePart: Array<string> = [];
              ...
              ...overwrite.map(
                (column) =>
                  `${this.escapeDatabaseIdentifier(
                    column
                  )} = EXCLUDED.${this.escapeDatabaseIdentifier(column)}`
              )
            );
          } else if (columns) {
            updatePart.push(
              ...columns.map(
                (column) =>
                  `${this.escapeDatabaseIdentifier(column)} = :${column}`
              )
            );
          }
          ...
          if (Array.isArray(overwrite) && skipUpdateIfNoValuesChanged) {
            ...
            const wheres = overwrite.map<WhereClause>((column) => ({
              type: 'or',
              condition: `${tableOrAliasName}.${this.escapeDatabaseIdentifier(
                column
              )} IS DISTINCT FROM EXCLUDED.${this.escapeDatabaseIdentifier(
                column
              )}`,
            }));
          ...
          if (
            DriverUtils.isPostgresFamily(this.connection.driver) &&
            this.expressionMap.onUpdate.overwriteCondition &&
            this.expressionMap.onUpdate.overwriteCondition.length > 0
          ) {
            query += ` WHERE ${this.createUpsertConditionExpression()}`;
          }
        }
      } else if (
        this.driver.supportedUpsertTypes.includes('on-duplicate-key-update')
      ) {
            ...
            query += overwrite
              .map(
                (column) =>
                  `${this.escapeDatabaseIdentifier(
                    column
                  )} = VALUES(${this.escapeDatabaseIdentifier(column)})`
              )
              .join(', ');
            query += ' ';
  ...
  protected createColumnNamesExpression(): string {
    ...
    if (columns.length > 0)
      return columns
        .map((column) => this.escapeDatabaseIdentifier(column.databaseName))
        .join(', ');
    ...
    if (
      ...
      if (valueSets.length === 1)
        return Object.keys(valueSets[0]!)
          .map((columnName) => this.escapeDatabaseIdentifier(columnName))
          .join(', ');
    }
  ...
  protected createValuesExpression(): string {
    ...
    if (columns.length > 0) {
      let expression = '';
      valueSets.forEach((_valueSet, valueSetIndex) => {
        columns.forEach((column, columnIndex) => {
          if (columnIndex === 0) {
            if (this.driver.options.type === 'oracle' && valueSets.length > 1) {
              expression += ' SELECT ';
            } else {
              expression += '(';
            }
          ...
          if (columnIndex === columns.length - 1) {
            if (valueSetIndex === valueSets.length - 1) {
              if (
                ...
                expression += ' FROM ' + this.driver.dummyTableName;
              } else {
                expression += ')';
              }
            } else {
              if (
                ...
                expression +=
                  ' FROM ' + this.driver.dummyTableName + ' UNION ALL ';
              } else {
                expression += '), ';
              }
            }
   ...
   */
  protected getValueSets(): Array<ObjectLiteral> {
  ...
  protected createMergeExpression(): string {
    if (!this.driver.supportedUpsertTypes.includes('merge-into'))
      throw new TypeORMError(
    ...
    const tableName = this.getTableName(this.getMainTableName());
    const tableAlias = this.escapeAlias(this.alias);
    const columns = this.getInsertedColumns();
    const columnsExpression = this.createColumnNamesExpression();

    let query = `MERGE INTO ${tableName} ${this.escapeAlias(this.alias)}`;

    const mergeSourceAlias = this.escapeAlias('mergeIntoSource');
    ...
    if (this.expressionMap.onIgnore) {
      ...
      if (primaryKey) {
        query += ` ON (${tableAlias}.${this.escapeDatabaseIdentifier(
          primaryKey.databaseName
        )} = ${mergeSourceAlias}.${this.escapeDatabaseIdentifier(
          primaryKey.databaseName
        )})`;
      } else {
        query += `ON (${this.expressionMap
          .mainAlias!.metadata.uniques.map((unique) => {
            return `(${unique.columns
              .map((column) => {
                return `${tableAlias}.${this.escapeDatabaseIdentifier(
                  column.databaseName
                )} = ${mergeSourceAlias}.${this.escapeDatabaseIdentifier(
                  column.databaseName
                )}`;
      ...
      if (Array.isArray(conflict)) {
        query += ` ON (${conflict
          .map(
            (column) =>
              `${tableAlias}.${this.escapeDatabaseIdentifier(
                column
              )} = ${mergeSourceAlias}.${this.escapeDatabaseIdentifier(column)}`
          )
          .join(' AND ')})`;
      } else if (conflict) {
        query += ` ON (${tableAlias}.${this.escapeDatabaseIdentifier(
          conflict
        )} = ${mergeSourceAlias}.${this.escapeDatabaseIdentifier(conflict)})`;
      } else {
        query += `ON (${this.expressionMap
          .mainAlias!.metadata.uniques.map((unique) => {
            return `(${unique.columns
    ...
    if (this.expressionMap.onUpdate) {
      const { overwrite, columns, conflict, skipUpdateIfNoValuesChanged } =
        this.expressionMap.onUpdate;
      ...
      if (Array.isArray(overwrite) && skipUpdateIfNoValuesChanged) {
        ...
        const wheres = overwrite.map<WhereClause>((column) => ({
          ...
          condition: {
            operator: 'notEqual',
            parameters: [
              `${tableAlias}.${this.escapeDatabaseIdentifier(column)}`,
              `${mergeSourceAlias}.${this.escapeDatabaseIdentifier(column)}`,
            ],
          },
        }));
        ...
        columns.forEach((column, columnIndex) => {
          ...
          if (
            ...
            !(
              ...
              !this.driver.isUUIDGenerationSupported()
            )
          ) {
            if (column.default !== undefined && column.default !== null) {
              // try to use default defined in the column
              expression += this.driver.normalizeDefault(column);
            } else {
    ...
    if (columns.length > 0) {
      columns.forEach((column, columnIndex) => {
        ...
        if (
          ...
          expression += `${mergeSourceAlias}.${this.escapeDatabaseIdentifier(
            column.databaseName
          )}`;
        }
  ...
  protected createColumnValueExpression(
    ...
    let value = column.getEntityValue(valueSet) as ObjectLiteral | undefined;
    ...
    if (!(typeof value === 'function')) {
      ...
      value = this.driver.preparePersistentValue(value, column) as
        | ObjectLiteral
        | undefined;
    }
    ...
    if (column.isVersion && value === undefined) {
      ...
      //     const tableName = this.driver.escape(column.entityMetadata.tablePath);
      //     const rightColumnName = this.driver.escape(column.entityMetadata.nestedSetRightColumn!.databaseName);
      ...
      value = randomUUID() as unknown as ObjectLiteral;
      expression += this.createParameter(value);
    ...
    } else if (value === null && this.driver.options.type === 'oracle') {
      expression += 'NULL';
      ...
      if (
        DriverUtils.isPostgresFamily(this.connection.driver) &&
        this.driver.spatialTypes.includes(column.type)
      ) {
        if (column.srid != null) {
      ...
      } else if (this.driver.spatialTypes.includes(column.type)) {
        expression +=
          column.type +
          '::STGeomFromText(' +
          paramName +
