...
> extends QueryBuilder<Entity> {
   ...
   */
  output(columns: Array<string>): this;
   ...
   * Optional returning/output clause.
   */
  output(output: string | Array<string>): this;
   ...
   * This will return given column values.
   */
  returning(columns: Array<string>): this;
   ...
   * Optional returning/output clause.
   */
  returning(returning: string | Array<string>): this;
      ...
      this.expressionMap
        .mainAlias!.metadata.columns.filter((column) =>
          this.expressionMap.insertColumns.length > 0
            ? this.expressionMap.insertColumns.indexOf(column.propertyPath) !==
              -1
            : column.isInsert
        )
      ...
      valueSets.forEach((valueSet, valueSetIndex) => {
        columns.forEach((column, columnIndex) => {
          ...
          if (columnIndex === columns.length - 1) {
            if (valueSetIndex === valueSets.length - 1) {
              if (
                ['oracle', 'sap'].includes(this.connection.driver.options.type)
              ) {
                expression += ' FROM ' + this.connection.driver.dummyTableName;
  ...
  protected createColumnValueExpression(
    ...
    if (column.isVersion && value === undefined) {
      ...
      if (this.connection.driver.options.type === 'mssql')
        value = (this.connection.driver as SqlServerDriver).parametrizeValue(
          column,
          value
        );
