...
{
  public override readonly '@instanceof' = Symbol.for('SelectQueryBuilder');
  ...
  public subQuery(): SelectQueryBuilder<ObjectLiteral> {
    ...
    qb.setIdentifierQuoting(this.expressionMap.identifierQuoting);
   ...
   * Replaces all previous selections if they exist.
   */
  public override select(selection: string, selectionAliasName?: string): this;

  /**
   * Creates SELECT query and selects given data.
   * Replaces all previous selections if they exist.
   */
  public override select(selection: Array<string>): this;
  ...
  public loadAllRelationIds(options?: {
    ...
    this.expressionMap.mainAlias!.metadata.relations.forEach((relation) => {
      if (options?.relations?.indexOf(relation.propertyPath) === -1) return;
   ...
   *
   * Entity column selections without custom aliases are normalized back to
   * entity property keys after driver-level result key transformation.
   */
  public async getRawMany<T = unknown>(): Promise<Array<T>> {
    ...
    try {
      ...
      const results = this.normalizeRawResultsToEntityProperties(
        await this.loadRawResults(queryRunner)
      ) as Array<T>;
  ...
  /**
   * Executes sql generated by query builder and returns raw results plus entities created from them.
   *
   * Raw result keys for entity column selections without custom aliases are
   * normalized back to entity property names.
   */
  ...
  public async stream(): Promise<Readable> {
    ...
    const queryRunner = this.obtainQueryRunner();
    let transactionStartedByUs = false;
    const ownsQueryRunner = queryRunner !== this.queryRunner;
    let finalized = false;
    const finalize = async (streamError?: Error): Promise<void> => {
      if (finalized) return;
      finalized = true;
      let finalizationError = streamError;

      if (transactionStartedByUs) {
        try {
          if (streamError) await queryRunner.rollbackTransaction();
      ...
      if (ownsQueryRunner) {
        try {
          await queryRunner.release(finalizationError);
        } catch (error: unknown) {
          finalizationError ??=
            error instanceof Error ? error : new Error(String(error));
        }
      ...
      if (!streamError && finalizationError) throw finalizationError;
    };
    try {
      // start transaction if it was enabled
      if (
      ...
      return await queryRunner.stream(sql, parameters, finalize, finalize);
    } catch (error: unknown) {
  ...
  protected join(
    ...
    if (joinAttributeMetadata) {
      ...
      joinAttribute.alias = this.expressionMap.createAlias({
        type: 'join',
        name: aliasName,
        tablePath: !isSubQuery ? entityOrProperty : undefined,
  ...
  protected createSelectExpression(): string {
    ...
    this.expressionMap.joinAttributes.forEach((join) => {
      if (join.metadata) {
        ...
        if (hasMainAlias) {
          allSelects.push({
            selection: this.escapeAlias(join.alias.name) + '.*',
          });
    ...
    this.expressionMap.selects
      .filter((select) => !excludedSelects.includes(select))
      .forEach((select) =>
        allSelects.push({
    ...
    const froms = this.expressionMap.aliases
      ...
      .map((alias) => {
        if (alias.subQuery)
          return alias.subQuery + ' ' + this.escapeAlias(alias.name);

        return (
          this.getTableName(alias.tablePath!) +
          ' ' +
          this.escapeAlias(alias.name)
        );
      });
    ...
    const selection = allSelects
      .map(
        (select) =>
          select.selection +
          (select.aliasName ? ' AS ' + this.escapeAlias(select.aliasName) : '')
      )
      .join(', ');
    ...
    const joins = this.expressionMap.joinAttributes.map((joinAttr) => {
      ...
      if (!parentAlias || !relation) {
        ...
        return (
          ...
          destinationJoin +
          ' ' +
          this.escapeAlias(destinationTableAlias) +
          this.createTableLockExpression() +
      ...
      if (relation.isManyToOne || relation.isOneToOneOwner) {
        ...
        return (
          ...
          ' ' +
          this.escapeAlias(junctionAlias) +
          this.createTableLockExpression() +
          ' ON ' +
          junctionCondition +
          ' ' +
    ...
    return (
      ...
      Object.keys(orderBys)
        .map((columnName) => {
          ...
          if (selection && !selection.aliasName && columnName.includes('.')) {
            ...
            if (alias?.hasMetadata) {
              ...
              if (column) {
                ...
                return this.escapeAlias(orderAlias) + ' ' + orderValue;
              }
            }
          }
  ...
  protected buildEscapedEntityColumnSelects(
      ...
      ...metadata.columns.filter((column) => {
        return this.expressionMap.selects.some(
          (select) =>
            ...
            select.selection === aliasName + '.' + column.databaseName ||
            select.selection === aliasName + '.' + column.databasePath
        );
      })
    );
    ...
    const nonSelectedPrimaryColumns = this.expressionMap.queryEntity
      ? metadata.primaryColumns.filter(
          (primaryColumn) => !columns.includes(primaryColumn)
        )
      : [];
    const allColumns = [...columns, ...nonSelectedPrimaryColumns];
    const finalSelects: Array<SelectQuery> = [];

    const escapedAliasName = this.escapeAlias(aliasName);
    allColumns.forEach((column) => {
      ...
      if (this.driver.spatialTypes.includes(column.type)) {
    ...
    if (DriverUtils.isPostgresFamily(this.connection.driver)) {
      ...
      return (
        ...
        primaryColumns
          .map(
            (c) =>
              `${distinctAlias}.${this.escapeDatabaseIdentifier(c.databaseName)}`
          )
          .join(', ') +
        '))'
      );
        ...
        if (this.findOptions.lock.mode === 'optimistic') {
          ...
          const tableNames = this.findOptions.lock.tables
            ? this.findOptions.lock.tables.map((table) => {
                ...
                return this.escapeAlias(tableAlias.name);
              })
            : undefined;
  ...
  protected async executeEntitiesAndRawResults(
    ...
    const relationIdLoader = new RelationIdLoader(
      ...
      this.expressionMap.identifierQuoting
    );
    ...
    if (
      ...
      const querySelects = metadata.primaryColumns.map((primaryColumn) => {
        const distinctAlias = this.escapeAlias('distinctAlias');
        const columnAlias = this.escapeAlias(
          DriverUtils.buildAlias(
            this.connection.driver,
            undefined,
            mainAliasName,
        ...
        return `${distinctAlias}.${columnAlias} AS ${this.escapeAlias(alias)}`;
      });
    ...
    if (rawResults.length > 0) {
      ...
      const transformer = new RawSqlResultsToEntityTransformer(
        ...
        this.queryRunner,
        this.connection.namingStrategy.transformColumnName.bind(
    ...
    if (this.expressionMap.relationLoadStrategy === 'query') {
      ...
      const relationLoadTasks = this.relationMetadatas.map(
        (relation) => async (): Promise<void> => {
          ...
          const queryBuilder = this.createQueryBuilder<
            SelectQueryBuilder<ObjectLiteral>
          >(queryRunner)
            .setIdentifierQuoting(this.expressionMap.identifierQuoting)
    ...
    return {
      raw: this.normalizeRawResultsToEntityProperties(rawResults),
    ...
    const selectString = Object.keys(orderBys)
      .map((orderCriteria) => {
        if (orderCriteria.includes('.')) {
          ...
          return (
            this.escapeAlias(parentAlias) + '.' + this.escapeAlias(builtAlias)
          );
        } else {
          if (
            ...
            return (
              this.escapeAlias(parentAlias) +
              '.' +
              this.escapeDatabaseIdentifier(orderCriteria)
            );
          }
  ...
  private normalizeRawResultsToEntityProperties(
    rawResults: Array<unknown>
  ): Array<unknown> {
    const rawEntityPropertyKeyMap = this.createRawEntityPropertyKeyMap();
    if (rawEntityPropertyKeyMap.size === 0) return rawResults;

    return rawResults.map((rawResult) => {
      if (!ObjectUtils.isObject(rawResult)) return rawResult;
      const normalizedResult: ObjectLiteral = {};

      for (const [key, value] of Object.entries(rawResult as ObjectLiteral)) {
        normalizedResult[rawEntityPropertyKeyMap.get(key) ?? key] = value;
      }
  ...
  private createRawEntityPropertyKeyMap(): Map<string, string> {
    const map = new Map<string, string>();
    const alias = this.expressionMap.mainAlias;
    if (!alias?.hasMetadata) return map;
    const shouldMapWholeAlias = this.expressionMap.selects.some(
      (select) => select.selection === alias.name && !select.aliasName
    );

    for (const column of alias.metadata.columns) {
      const shouldMapColumnAlias =
        shouldMapWholeAlias ||
        this.expressionMap.selects.some(
          (select) =>
            !select.aliasName &&
            (select.selection === `${alias.name}.${column.propertyPath}` ||
              select.selection === `${alias.name}.${column.databaseName}` ||
              select.selection === `${alias.name}.${column.databasePath}`)
        );
      if (!shouldMapColumnAlias) continue;

      const rawAlias = DriverUtils.buildAlias(
        this.connection.driver,
        undefined,
        alias.name,
        column.databaseName
      );
      map.set(rawAlias, column.propertyPath);

      const transformedRawAlias =
        this.connection.namingStrategy.transformColumnName(rawAlias);
      map.set(transformedRawAlias, column.propertyPath);
    }
   ...
   */
  protected override obtainQueryRunner(): QueryRunner {
    return (
      this.queryRunner ||
    ...
    if (Array.isArray(where)) {
      ...
      for (const key in where) {
        ...
        if (column) {
          ...
          if (column.isVirtualProperty && column.query) {
            aliasPath = `(${column.query(this.escapeAlias(alias))})`;
          }
