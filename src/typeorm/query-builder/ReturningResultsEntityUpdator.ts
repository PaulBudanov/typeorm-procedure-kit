...
export class ReturningResultsEntityUpdator {
  ...
  public async update(
    ...
    await Promise.all(
      entities.map(async (entity, entityIndex) => {
        ...
        if (
          this.queryRunner.connection.driver.isReturningSqlSupported('update')
        ) {
          updateResult.raw = this.mapOracleReturningResult(
            updateResult.raw,
            'update'
          );
          const result = Array.isArray(updateResult.raw)
            ? (updateResult.raw[entityIndex] as unknown)
            : updateResult.raw;
  ...
  public async insert(
    ...
    const generatedMaps = entities.map((entity, entityIndex) => {
      insertResult.raw = this.mapOracleReturningResult(
        insertResult.raw,
        'insert'
      );
      ...
      const result = Array.isArray(insertResult.raw)
        ? (insertResult.raw[entityIndex] as unknown)
        : insertResult.raw;
    ...
    entities.forEach((entity, entityIndex) => {
      ...
      insertResult.identifiers.push(entityIdMap!);
      insertResult.generatedMaps.push(generatedMaps[entityIndex]!);
    });
  }

  private mapOracleReturningResult(
    raw: unknown,
    ...
    if (raw.length !== columns.length) {
      throw new TypeORMError(
        'Oracle RETURNING result does not match the requested columns'
      );
    }
    ...
    for (const [index, column] of columns.entries()) {
      ...
      if (!Array.isArray(values)) {
        throw new TypeORMError(
          'Oracle RETURNING column must contain an array of values'
        );
      }
      result[column.databaseName] = values[0];
    }
    return result;
  }
   ...
   */
  public getUpdationReturningColumns(): Array<ColumnMetadata> {
   ...
   */
  public getSoftDeletionReturningColumns(): Array<ColumnMetadata> {
    return this.expressionMap.mainAlias!.metadata.columns.filter((column) => {
      return (
        column.asExpression !== undefined ||
        column.isUpdateDate ||
