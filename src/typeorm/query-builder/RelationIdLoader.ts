...
export class RelationIdLoader {
  ...
  >(
    ...
    const entities: Array<E1> = Array.isArray(entitiesOrEntities)
      ? entitiesOrEntities
      : [entitiesOrEntities];
    ...
    return entities.map((entity) => {
      const group: { entity: E1; related?: E2 | Array<E2> } = {
        entity: entity,
        related: isMany ? [] : undefined,
      };
      ...
      const entityRelationIds = relationIds.filter((relationId) => {
        return inverseColumns.every((column) => {
          return column.compareEntityValue(
            entity,
            relationId[
              column.entityMetadata.name + '_' + column.propertyAliasName
            ]
          );
        });
      });
      ...
      _relatedEntities.forEach((relatedEntity) => {
        entityRelationIds.forEach((relationId) => {
          ...
          if (relatedEntityMatched) {
            if (isMany) {
              (group.related as Array<E2>).push(relatedEntity);
            } else {
  ...
  protected loadForManyToOneAndOneToOneOwner(
    ...
    if (relatedEntities && hasAllJoinColumnsInEntity) {
      const relationIdMaps: Array<ObjectLiteral> = [];
        ...
        .map((entity, entityIndex) => {
          return relation.joinColumns
            .map((joinColumn, joinColumnIndex) => {
              ...
              return (
                mainAlias + '.' + joinColumn.propertyPath + ' = :' + paramName
              );
            })
            .join(' AND ');
        })
