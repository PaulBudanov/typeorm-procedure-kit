...
> extends Repository<Entity> {
   ...
   * Finds entities that match given find options or conditions.
   */
  findBy(where: any): Promise<Array<Entity>> {
   ...
   * but ignores pagination settings (from and take options).
   */
  findAndCountBy(where: any): Promise<[Array<Entity>, number]> {
   ...
   * })
   */
  findByIds(ids: Array<any>, options?: any): Promise<Array<Entity>> {
   ...
   */
  async findOne(options: MongoFindOneOptions<Entity>): Promise<Entity | null> {
   ...
   */
  bulkWrite(
    operations: Array<AnyBulkWriteOperation>,
   ...
   * Retrieve all the indexes on the collection.
   */
  collectionIndexExists(indexes: string | Array<string>): Promise<boolean> {
