...
import { EntityListenerMetadata } from '../metadata/EntityListenerMetadata.js';
import { EntityMetadata } from '../metadata/EntityMetadata.js';
import { ExclusionMetadata } from '../metadata/ExclusionMetadata.js';
...
export class EntityMetadataBuilder {
   ...
   */
  public build(entityClasses?: Array<TFunction>): Array<EntityMetadata> {
    ...
    entityMetadatas
      ...
      .forEach((entityMetadata) => {
        ...
        entityMetadata.relations
          ...
          .forEach((relation) => {
            const joinColumns = this.metadataArgsStorage.filterJoinColumns(
              relation.target,
        ...
        entityMetadata.relations
          ...
          .forEach((relation) => {
            const joinTable = this.metadataArgsStorage.findJoinTable(
              relation.target,
              relation.propertyName
            )!;
            ...
            const junctionEntityMetadata =
              this.junctionEntityMetadataBuilder.build(relation, joinTable);
    ...
    entityMetadatas.forEach((entityMetadata) => {
      entityMetadata.relationsWithJoinColumns = entityMetadata.relations.filter(
    ...
    entityMetadatas
      ...
      .forEach((entityMetadata) => {
        const closureJunctionEntityMetadata =
          this.closureJunctionEntityMetadataBuilder.build(entityMetadata);
  ...
  protected createEntityMetadata(tableArgs: TableMetadataArgs): EntityMetadata {
    ...
    if (
      ...
      inheritanceTree.push(...(singleTableChildrenTargets as Array<TFunction>));
    }

    return new EntityMetadata({
      connection: this.connection,
      args: tableArgs,
      inheritanceTree: inheritanceTree as Array<TFunction>,
      tableTree: tableTree,
  ...
  protected computeEntityMetadataStep1(
    ...
    const discriminatorValue = this.metadataArgsStorage.findDiscriminatorValue(
      entityMetadata.target
    );

    if (typeof discriminatorValue !== 'undefined') {
      entityMetadata.discriminatorValue = discriminatorValue.value as string;
    } else {
  ...
  protected createEmbeddedsRecursively(
    entityMetadata: EntityMetadata,
    embeddedArgs: Array<EmbeddedMetadataArgs>
    ...
    return embeddedArgs.map((embeddedArgs) => {
      ...
      embeddedMetadata.embeddeds.forEach(
        (subEmbedded) => (subEmbedded.parentEmbeddedMetadata = embeddedMetadata)
      );
  ...
  protected computeEntityMetadataStep2(entityMetadata: EntityMetadata): void {
    ...
    entityMetadata.ownerManyToManyRelations = entityMetadata.relations.filter(
    ...
    entityMetadata.listeners = entityMetadata.embeddeds.reduce(
      (listeners, embedded) => listeners.concat(embedded.listenersFromTree),
  ...
  protected computeInverseProperties(
    ...
    entityMetadata.relations.forEach((relation) => {
      ...
      relation.inverseRelation = inverseEntityMetadata.relations.find(
        (foundRelation) =>
          foundRelation.propertyPath === relation.inverseSidePropertyPath
      );
    });
  }
  ...
  protected createKeysForTableInheritance(
    ...
    const isDiscriminatorColumnAlreadyIndexed = entityMetadata.indices.some(
      ({ givenColumnNames }) =>
        ...
        givenColumnNames[0] === entityMetadata.discriminatorColumn?.databaseName
    );
  ...
  protected createForeignKeys(
    ...
    this.metadataArgsStorage
      ...
      .forEach((foreignKeyArgs) => {
        ...
        const referencedColumns: Array<ColumnMetadata> = [];
        ...
        referencedColumns.push(
          ...referencedColumnNames.map((columnName) =>
            columnNameToColumn(columnName, referencedEntityMetadata)
          )
        );
