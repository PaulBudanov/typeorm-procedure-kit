...
export class EntityPersistExecutor {
  ...
  public async execute(): Promise<void> {
    ...
    try {
      ...
      const executors = await OrmUtils.executeTasks(
        entitiesInChunks.map(
          (entities) => async (): Promise<SubjectExecutor> => {
            ...
            if (
              ...
              subjects.forEach((subject) => {
                if (subject.mustBeRemoved) {
                  new ManyToManySubjectBuilder(subjects).buildForAllRemoval(
                    subject
                  );
                }
              });
      ...
      try {
        ...
        // console.time("commit");
        if (isTransactionStartedByUs) await queryRunner.commitTransaction();
