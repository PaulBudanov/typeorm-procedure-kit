...
export class MigrationExecutor {
   ...
   */
  public async getExecutedMigrations(): Promise<Array<Migration>> {
    return this.withQueryRunner(async (queryRunner) => {
      await this.createMigrationsTableIfNotExist(queryRunner);
   ...
   */
  public async getPendingMigrations(): Promise<Array<Migration>> {
   ...
   */
  public async executePendingMigrations(): Promise<Array<Migration>> {
    const queryRunner = this.queryRunner ?? this.connection.createQueryRunner();
    ...
    if (this.transaction === 'all') {
      ...
      const migrationsOverridingTransactionMode = pendingMigrations.filter(
        (migration) => !(migration.instance?.transaction === undefined)
      );
  ...
  protected getLatestExecutedMigration(
    sortedMigrations: Array<Migration>
  ): Migration | undefined {
    return sortedMigrations.length > 0 ? sortedMigrations[0] : undefined;
  }
