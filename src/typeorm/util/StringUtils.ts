...
export function camelCase(str: string, firstCapital = false): string {
  ...
  return str.replace(
    /^([A-Z])|[\s-_](\w)/g,
    function (_match, p1: string, p2: string | undefined) {
...
export function shorten(input: string, options: IShortenOptions = {}): string {
  ...
  const shortSegments = segments.reduce((acc: Array<string>, val: string) => {
...
/**
 * Returns a SHA-256 hex digest for internal IDs/aliases.
 *
